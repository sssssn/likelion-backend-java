package com.likelion.project02.week9.day1;
...
public class ParTest2 {
    public boolean solution() throws IOException {
        ...
        for (int i = 0; i < input.length(); i++) {
            ...
            // 1. 모든 여는 괄호를 만날 경우 push
            if (next == '(' || next == '{' || next == '[') {
                charStack.push(next);
            }
            // 2. 어떤 닫는 괄호든 만날 경우
            else if (next == ')' || next == '}' || next == ']') {
                // a. pop 없으면 검사 실패 (false 반환) = 스택이 비어있다.
                if (charStack.empty()) return false;
                // b. 아니라면 pop
                char top = charStack.pop();
                // c. pop의 결과로 나온 값이 올바른 여는 괄호인지 확인 = '들어온건 닫는 소괄호 && top은 여는 소괄호'가 아니라면 실패
